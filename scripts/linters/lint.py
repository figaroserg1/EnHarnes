...
Auto-detects project type and runs the appropriate linter:
  ...
  - Python (pyproject.toml/requirements.txt) → custom_linter.py + dependency_guard.py
...
def main() -> int:
    ...
    if (ROOT / "pyproject.toml").exists() or (ROOT / "requirements.txt").exists():
        py = sys.executable
        rc1 = run([py, "scripts/linters/custom_linter.py"])
        rc2 = run([py, "scripts/linters/dependency_guard.py"])
        return max(rc1, rc2)

    print("No default lint command detected.")
    print("Set HARNESS_LINT_CMD or customize scripts/linters/lint.py.")
    return 1
