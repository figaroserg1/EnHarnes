...
If a principle cannot be enforced automatically, it belongs in `docs/harness/WORKFLOW_RULES.md` instead.
...
Enforced by: `scripts/harness/architecture/test_layer_dependencies.py`
...
Enforced by: `scripts/harness/code-quality/code_conventions.py` (Rule 4: bare print() detection in src/)
...
Enforced by: `scripts/harness/code-quality/code_conventions.py` (Rule 5: file size limits on src/ files)

**6. Every TODO in a markdown file must have an owner tag.**
Format: `TODO: [HUMAN]`, `TODO: [AI]`, or `TODO: [AI->HUMAN]`
Enforced by: `scripts/harness/doc-health/doc_linter.py`
...
Enforced by: `scripts/harness/doc-health/doc_linter.py` (planned: AST rule scanning dynamic key access outside schema modules)
...
Enforced by: `scripts/harness/architecture/test_duplicate_helpers.py` (similarity scan or import whitelist)
...
Enforced by: `scripts/harness/architecture/test_side_effect_patterns.py` (scan Runtime/Service layers for external clients without idempotency wrapper)
...
Enforced by: `scripts/harness/doc-health/doc_linter.py` (planned: check function signatures or logging context usage)
...
Enforced by: `scripts/harness/architecture/test_contract_changes.py` (diff-based schema/API check)
...
Enforced by: `scripts/harness/code-quality/code_conventions.py` (planned: check that service modules contain logging setup)
...
Enforced by: `scripts/harness/code-quality/code_conventions.py` (naming convention checks on src/ files)
...
|-----------|----------|------|
| 1. Layer imports | `scripts/harness/architecture/test_layer_dependencies.py` | `make structural` |
| 2. Boundary validation | structural test + code review | `make structural` |
| 3. No biz logic in UI | `scripts/harness/architecture/test_layer_dependencies.py` | `make structural` |
| 4. Structured logging | `scripts/harness/code-quality/code_conventions.py` | `make check` |
| 5. File size limits | `scripts/harness/code-quality/code_conventions.py` | `make check` |
| 6. TODO ownership | `scripts/harness/doc-health/doc_linter.py` | `make smoke` |
| 7. No data probing | `scripts/harness/doc-health/doc_linter.py` (planned) | `make check` |
| 8. Shared utilities | `scripts/harness/architecture/test_duplicate_helpers.py` (planned) | `make structural` |
| 9. Idempotent side-effects | `scripts/harness/architecture/test_side_effect_patterns.py` (planned) | `make structural` |
| 10. Trace propagation | `scripts/harness/doc-health/doc_linter.py` (planned) | `make check` |
| 11. Contract versioning | `scripts/harness/architecture/test_contract_changes.py` (planned) | `make structural` |
| 12. Providers only | `scripts/harness/architecture/test_layer_dependencies.py` | `make structural` |
| 13. Detailed logging | `scripts/harness/code-quality/code_conventions.py` (planned) | `make check` |
| Naming conventions | `scripts/harness/code-quality/code_conventions.py` | `make smoke` |
...
lives in `docs/harness/WORKFLOW_RULES.md`. It guides judgment — it is not a linter rule.
